...
%                          Parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [u, error, count, Energy, E_tot] = Split_Bregman_Combined(par, vals, dd, g, Lwr, Upr,u0,example,potential)
...
E_grad1 = (uold-u).^2/(2*h) + 2*a*u.^2 -(4*uold.^3 -4*uold.*(g+a)).*(u-uold);
E_grad2 = (lmb/2)*(v-ux-b).^2;

E_grad = sum((E_grad1(1:end-1) + E_grad1(2:end))*(dx/2))+...
    sum((E_grad2(1:end-1) + E_grad2(2:end))*(dx/2));
